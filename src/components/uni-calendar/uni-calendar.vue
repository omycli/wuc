  ...
  methods: {
    ...
    getMonthAll(index, date, first) {
      ...
      let beforeyear =
        ...
        canlender.month +
        "-" +
        (canlender.date > tempbefore.lastDate
          ? tempbefore.lastDate
          : canlender.date);
      let afteryear =
        canlender.year +
        ...
        canlender.month +
        "-" +
        (canlender.date > tempafter.lastDate
          ? tempafter.lastDate
          : canlender.date);
      ...
      if (this.disableBefore) {
        if (!this.startDate) {
          if (!this.endDate) {
            ...
            return (
              !this.dateCompare(this.getDate(this.date, 0), time) || endDate
            );
          }
        } else {
          return (
            !this.dateCompare(this.getDate(this.date, 0), time) ||
            !startDate ||
            endDate
          );
        }
...
@font-face {
  ...
  /* IE9*/
  src: url("//at.alicdn.com/t/font_989023_qdgy7euvg4.eot?t=1545961121132#iefix")
    ...
    /* IE6-IE8 */
      ...
      format("woff"),
    url("//at.alicdn.com/t/font_989023_qdgy7euvg4.ttf?t=1545961121132")
      format("truetype"),
    /* chrome, firefox, opera, Safari, Android, iOS 4.2+*/
      url("//at.alicdn.com/t/font_989023_qdgy7euvg4.svg?t=1545961121132#iconfont")
      format("svg");
  /* iOS 4.1- */
}
